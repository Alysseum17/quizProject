...
export default class QuizService {
  private async verifyQuizOwnership(
    tx: any,
    quizId: number,
    userId: number
  ): Promise<void> {
  ...
  private calculateAnswerScore(
    ...
    if (question.question_type === "free_text") {
      ...
      const correctAnswerText = correctAnswerOption
        ? correctAnswerOption.answer_text
        : "";
      if (
        free_text_answer &&
        free_text_answer.toLowerCase().trim() ===
      ...
      if (selected_option_ids) {
        const correctOptions = question.answer_options.filter(
          (o: any) => o.is_correct
        );
        ...
        for (const selectedId of selected_option_ids) {
          ...
          if (!option) {
            throw new AppError(
              `Option ID ${selectedId} does not belong to question ${question.id}`,
              400
            );
          }
        ...
        if (totalCorrectOptionsCount === 0) {
          ...
          answerScore =
            question.points *
            (correctlySelectedCount / totalCorrectOptionsCount);
        }
      }
    }
    return answerScore;
  ...
  async findQuizByName(name: string) {
    ...
    return quizzes.map((quiz) => {
      const totalRatings = quiz.reviews.reduce(
        (sum, review) => sum + Number(review.rating),
        0
      );
      const averageRating =
        quiz.reviews.length > 0 ? totalRatings / quiz.reviews.length : 0;
      return {
        quiz_id: quiz.id,
        title: quiz.title,
  ...
  async getSortedQuizByRating(query: SortedQuizByRatingQuery) {
    const sortDirection =
      query.sort === "asc" ? Prisma.sql`ASC` : Prisma.sql`DESC`;
  ...
  async submitQuizAttempt(attemptId: number, answers: Answer[]) {
    return await prisma.$transaction(async (tx) => {
      ...
      for (const answer of answers) {
        ...
        answerScore = this.calculateAnswerScore(
          question,
          selected_option_ids,
          free_text_answer
        );
  ...
  async getQuizResults(attemptId: number) {
    ...
    const totalPointsEarned = attempt.question_responses.reduce(
      (sum, response) =>
        response.earned_points ? sum + response.earned_points : sum,
      0
    );
    const totalPossiblePoints = attempt.question_responses.reduce(
